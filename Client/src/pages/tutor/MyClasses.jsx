...
export default function MyClasses() {
  ...
  const uniqueSubjects = [...new Set(classes.map(cls => cls.subjectId?.name).filter(Boolean))];
  const uniqueBatches = [...new Set(classes.map(cls => cls.batchId?.name).filter(Boolean))];
  ...
  const filteredClasses = classes.filter(cls => {
    ...
    const matchesSubject = filters.subject === ALL_VALUE || cls.subjectId?.name === filters.subject;
    const matchesBatch = filters.batch === ALL_VALUE || cls.batchId?.name === filters.batch;
    const matchesStatus = filters.status === ALL_VALUE || 
      (filters.status === "Scheduled" && cls.status !== "completed" && cls.status !== "cancelled") ||
    ...
    return matchesTopic && matchesSubject && matchesBatch && matchesSchedule && matchesStatus;
  });
                  ...
                  <TableBody>
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="text-center text-sm">
                        {classes.length === 0 ? "No classes assigned" : "No classes match the current filters"}
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
